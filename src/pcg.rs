...
impl SeedFromRng for PcgXsl128LcgRng {
    fn from_rng<R: Rng>(mut other: R) -> Result<Self, Error> {
        // We only have to make sure increment is odd.
        let mut ctx = PcgXsl128LcgRng { state: (other.next_u64() as u128) << 64 |
                                               (other.next_u64() as u128) };
        // Prepare for the first round
        ctx.state = ctx.state.wrapping_mul(MULTIPLIER).wrapping_add(INCREMENT);
        Ok(ctx)
    }
}

impl Rng for PcgXsl128LcgRng {
    #[inline]
    fn next_u32(&mut self) -> u32 {
